		...
		<?php foreach ($list as $item) : ?>
			<?php
				...
				if (empty($item->image_first)) {
					...
					if (!empty($result_array[0]) && empty($result_array[1])) {

						if ($center_vertically) {
							$imagesize = @getimagesize($result_array[0]);
							if ($imagesize !== FALSE) {
								$imageheight = $imagesize[1];

								$top = intval(($height - $imageheight) / 2);
								$extra_style .= '; top: '.$top.'px"';
							}
						}

						if ($create_highres_images) {
							//$filename_highres = str_replace('.', '@2x.', Uri::base(true).'/'.$result_array[0]);
							//$extra_attribute = ' data-src="'.$filename_highres.'"';

							$attributes['srcset'] = 'data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==';
							$attributes['data-srcset'] = Uri::base(true).'/'.$result_array[0].' 1x, '.str_replace('.', '@2x.', Uri::base(true).'/'.$result_array[0]).' 2x';

							$extra_class .= ' lazyload';
						}
					}

					if (!empty($result_array[0])) {
						//$imagetag .= '<img class="original" src="'.Uri::base(true).'/'.$result_array[0].'"'.$extra_attribute.$extra_style.' border="0" alt="'.$item->alt_first.'" />';

						$attributes['class'] = 'original'.$extra_class;
						$attributes['style'] = 'border: 0'.$extra_style;

						$imagetag .= HTMLHelper::_('image', $result_array[0], $item->alt_first, $attributes);
					}
					...
					if (!empty($result_array_hover[0]) && empty($result_array_hover[1])) {

						if ($center_vertically) {
							$imagesize = @getimagesize($result_array_hover[0]);
							if ($imagesize !== FALSE) {
								$imageheight = $imagesize[1];
						...
						if ($create_highres_images) {
							//$filename_highres = str_replace('.', '@2x.', Uri::base(true).'/'.$result_array_hover[0]);
							//$extra_attribute = ' data-src="'.$filename_highres.'"';

							$attributes['srcset'] = 'data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==';
							$attributes['data-srcset'] = Uri::base(true).'/'.$result_array_hover[0].' 1x, '.str_replace('.', '@2x.', Uri::base(true).'/'.$result_array_hover[0]).' 2x';
					...
					if (!empty($result_array_hover[0])) {
						if ($params->get('logo_tooltip', 1)) {
							if ($bootstrap_version > 0) {
								...
								$extra_class .= ' hasTooltip';
							}
							$attributes['title'] = htmlspecialchars($item->title);
							//$imagetag .= '<img class="hover hasTooltip" src="'.Uri::base(true).'/'.$result_array_hover[0].'"'.$extra_attribute.$extra_style.' border="0" alt="'.$item->alt_first.'" title="'.$item->title.'" />';
						//} else {
							//$imagetag .= '<img class="hover" src="'.Uri::base(true).'/'.$result_array_hover[0].'"'.$extra_attribute.$extra_style.' border="0" alt="'.$item->alt_first.'" />';
						}

						$attributes['class'] = 'hover'.$extra_class;
						$attributes['style'] = 'border: 0'.$extra_style;

						$imagetag .= HTMLHelper::_('image', $result_array_hover[0], $item->alt_first, $attributes);
					}
				}
			?>
			<li class="weblink_item weblink_id_<?php echo $item->id; ?> weblink_catid_<?php echo $item->catid; ?>">
				...
				<?php else : ?>
					...
					<div class="weblink_item_wrapper">
						...
						<?php if ($params->get('description', 0) || $params->get('title', 0) || $params->get('hits', 0)) : ?>
