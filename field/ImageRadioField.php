...
class ImageRadioField extends FormField
{
    ...
    protected function getInput()
    {
        ...
        $class     = !empty($this->class) ? ' class="form-check form-check-inline ' . $this->class . '"' : ' class="form-check form-check-inline"';
        ...
        foreach ($options as $i => $option) {
            ...
            $html[] = '<input type="radio" id="' . $this->id . $i . '" name="' . $this->name . '" value="'
                . htmlspecialchars($option->value, ENT_COMPAT, 'UTF-8') . '"' . $checked . $required . $onclick . 'class="form-check-input'.$class.'"'
                    . $onchange . $disabled . ' />';
                    
                    $title =  Text::alt($option->text, preg_replace('/[^a-zA-Z0-9_\-]/', '_', $this->fieldname));
                    $html[] = '<label title="'. $title .'" for="' . $this->id . $i . '" class="form-check-label hasTooltip'.$class.'">';
